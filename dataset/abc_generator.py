...
def abc_query(year, start, year_delta=1, **kwargs):
    return query("ABC News", year, start, year_delta, **kwargs)

def abc_scraper(html):
    soup = BeautifulSoup(html, 'html5lib')

    content = soup.find('section', {'class': 'story'})
    date = soup.find('div', {'class': 'Byline__Meta--publishDate'})
...
data = []
if path.exists('./abc_data.json'):
    with open('./abc_data.json', 'r') as f:
        data = json.load(f)
...
while requests < max_requests:
    ...
    while count <= max_count and count < 100 and requests < max_requests:
        ...
        max_count, articles = abc_query(year, count, year_delta)
        ...
        for r, m in zip(responses, articles):
            if r:
                m['date'], m['text'] = abc_scraper(r.content)
                if m['text'] == '' or m['date'] == '':
